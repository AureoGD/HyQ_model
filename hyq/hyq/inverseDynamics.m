function [tau trunk_a] = inverseDynamics(ip, xm, trunk_v, gravity, qd, qdd, q)
...
LF_hipassembly_v = zeros(6,1);
LF_hipassembly_a = zeros(6,1);

LF_hipassembly_v = xm.fr_LF_hipassembly_XM_fr_trunk * trunk_v;
LF_hipassembly_v(3) += qd(1);

vcross = vcross_mx(LF_hipassembly_v);

LF_hipassembly_a = vcross(:,3) * qd(1);
LF_hipassembly_a(3) += qdd(1);

LF_hipassembly_f = ip.lf_LF_hipassembly.tensor6D * LF_hipassembly_a + (-vcross' * ip.lf_LF_hipassembly.tensor6D * LF_hipassembly_v);

% First pass, link 'LF_upperleg'
LF_upperleg_v = zeros(6,1);
LF_upperleg_a = zeros(6,1);

LF_upperleg_v = xm.fr_LF_upperleg_XM_fr_LF_hipassembly * LF_hipassembly_v;
LF_upperleg_v(3) += qd(2);

vcross = vcross_mx(LF_upperleg_v);

LF_upperleg_a = xm.fr_LF_upperleg_XM_fr_LF_hipassembly * LF_hipassembly_a + (vcross(:,3) * qd(2));
LF_upperleg_a(3) += qdd(2);

LF_upperleg_f = ip.lf_LF_upperleg.tensor6D * LF_upperleg_a + (-vcross' * ip.lf_LF_upperleg.tensor6D * LF_upperleg_v);

% First pass, link 'LF_lowerleg'
LF_lowerleg_v = zeros(6,1);
LF_lowerleg_a = zeros(6,1);

LF_lowerleg_v = xm.fr_LF_lowerleg_XM_fr_LF_upperleg * LF_upperleg_v;
LF_lowerleg_v(3) += qd(3);

vcross = vcross_mx(LF_lowerleg_v);

LF_lowerleg_a = xm.fr_LF_lowerleg_XM_fr_LF_upperleg * LF_upperleg_a + (vcross(:,3) * qd(3));
LF_lowerleg_a(3) += qdd(3);

LF_lowerleg_f = ip.lf_LF_lowerleg.tensor6D * LF_lowerleg_a + (-vcross' * ip.lf_LF_lowerleg.tensor6D * LF_lowerleg_v);

% First pass, link 'RF_hipassembly'
RF_hipassembly_v = zeros(6,1);
RF_hipassembly_a = zeros(6,1);

RF_hipassembly_v = xm.fr_RF_hipassembly_XM_fr_trunk * trunk_v;
RF_hipassembly_v(3) += qd(4);

vcross = vcross_mx(RF_hipassembly_v);

RF_hipassembly_a = vcross(:,3) * qd(4);
RF_hipassembly_a(3) += qdd(4);

RF_hipassembly_f = ip.lf_RF_hipassembly.tensor6D * RF_hipassembly_a + (-vcross' * ip.lf_RF_hipassembly.tensor6D * RF_hipassembly_v);

% First pass, link 'RF_upperleg'
RF_upperleg_v = zeros(6,1);
RF_upperleg_a = zeros(6,1);

RF_upperleg_v = xm.fr_RF_upperleg_XM_fr_RF_hipassembly * RF_hipassembly_v;
RF_upperleg_v(3) += qd(5);

vcross = vcross_mx(RF_upperleg_v);

RF_upperleg_a = xm.fr_RF_upperleg_XM_fr_RF_hipassembly * RF_hipassembly_a + (vcross(:,3) * qd(5));
RF_upperleg_a(3) += qdd(5);

RF_upperleg_f = ip.lf_RF_upperleg.tensor6D * RF_upperleg_a + (-vcross' * ip.lf_RF_upperleg.tensor6D * RF_upperleg_v);

% First pass, link 'RF_lowerleg'
RF_lowerleg_v = zeros(6,1);
RF_lowerleg_a = zeros(6,1);

RF_lowerleg_v = xm.fr_RF_lowerleg_XM_fr_RF_upperleg * RF_upperleg_v;
RF_lowerleg_v(3) += qd(6);

vcross = vcross_mx(RF_lowerleg_v);

RF_lowerleg_a = xm.fr_RF_lowerleg_XM_fr_RF_upperleg * RF_upperleg_a + (vcross(:,3) * qd(6));
RF_lowerleg_a(3) += qdd(6);

RF_lowerleg_f = ip.lf_RF_lowerleg.tensor6D * RF_lowerleg_a + (-vcross' * ip.lf_RF_lowerleg.tensor6D * RF_lowerleg_v);

% First pass, link 'LH_hipassembly'
LH_hipassembly_v = zeros(6,1);
LH_hipassembly_a = zeros(6,1);

LH_hipassembly_v = xm.fr_LH_hipassembly_XM_fr_trunk * trunk_v;
LH_hipassembly_v(3) += qd(7);

vcross = vcross_mx(LH_hipassembly_v);

LH_hipassembly_a = vcross(:,3) * qd(7);
LH_hipassembly_a(3) += qdd(7);

LH_hipassembly_f = ip.lf_LH_hipassembly.tensor6D * LH_hipassembly_a + (-vcross' * ip.lf_LH_hipassembly.tensor6D * LH_hipassembly_v);

% First pass, link 'LH_upperleg'
LH_upperleg_v = zeros(6,1);
LH_upperleg_a = zeros(6,1);

LH_upperleg_v = xm.fr_LH_upperleg_XM_fr_LH_hipassembly * LH_hipassembly_v;
LH_upperleg_v(3) += qd(8);

vcross = vcross_mx(LH_upperleg_v);

LH_upperleg_a = xm.fr_LH_upperleg_XM_fr_LH_hipassembly * LH_hipassembly_a + (vcross(:,3) * qd(8));
LH_upperleg_a(3) += qdd(8);

LH_upperleg_f = ip.lf_LH_upperleg.tensor6D * LH_upperleg_a + (-vcross' * ip.lf_LH_upperleg.tensor6D * LH_upperleg_v);

% First pass, link 'LH_lowerleg'
LH_lowerleg_v = zeros(6,1);
LH_lowerleg_a = zeros(6,1);

LH_lowerleg_v = xm.fr_LH_lowerleg_XM_fr_LH_upperleg * LH_upperleg_v;
LH_lowerleg_v(3) += qd(9);

vcross = vcross_mx(LH_lowerleg_v);

LH_lowerleg_a = xm.fr_LH_lowerleg_XM_fr_LH_upperleg * LH_upperleg_a + (vcross(:,3) * qd(9));
LH_lowerleg_a(3) += qdd(9);

LH_lowerleg_f = ip.lf_LH_lowerleg.tensor6D * LH_lowerleg_a + (-vcross' * ip.lf_LH_lowerleg.tensor6D * LH_lowerleg_v);

% First pass, link 'RH_hipassembly'
RH_hipassembly_v = zeros(6,1);
RH_hipassembly_a = zeros(6,1);

RH_hipassembly_v = xm.fr_RH_hipassembly_XM_fr_trunk * trunk_v;
RH_hipassembly_v(3) += qd(10);

vcross = vcross_mx(RH_hipassembly_v);

RH_hipassembly_a = vcross(:,3) * qd(10);
RH_hipassembly_a(3) += qdd(10);

RH_hipassembly_f = ip.lf_RH_hipassembly.tensor6D * RH_hipassembly_a + (-vcross' * ip.lf_RH_hipassembly.tensor6D * RH_hipassembly_v);

% First pass, link 'RH_upperleg'
RH_upperleg_v = zeros(6,1);
RH_upperleg_a = zeros(6,1);

RH_upperleg_v = xm.fr_RH_upperleg_XM_fr_RH_hipassembly * RH_hipassembly_v;
RH_upperleg_v(3) += qd(11);

vcross = vcross_mx(RH_upperleg_v);

RH_upperleg_a = xm.fr_RH_upperleg_XM_fr_RH_hipassembly * RH_hipassembly_a + (vcross(:,3) * qd(11));
RH_upperleg_a(3) += qdd(11);

RH_upperleg_f = ip.lf_RH_upperleg.tensor6D * RH_upperleg_a + (-vcross' * ip.lf_RH_upperleg.tensor6D * RH_upperleg_v);

% First pass, link 'RH_lowerleg'
RH_lowerleg_v = zeros(6,1);
RH_lowerleg_a = zeros(6,1);

RH_lowerleg_v = xm.fr_RH_lowerleg_XM_fr_RH_upperleg * RH_upperleg_v;
RH_lowerleg_v(3) += qd(12);

vcross = vcross_mx(RH_lowerleg_v);

RH_lowerleg_a = xm.fr_RH_lowerleg_XM_fr_RH_upperleg * RH_upperleg_a + (vcross(:,3) * qd(12));
RH_lowerleg_a(3) += qdd(12);

RH_lowerleg_f = ip.lf_RH_lowerleg.tensor6D * RH_lowerleg_a + (-vcross' * ip.lf_RH_lowerleg.tensor6D * RH_lowerleg_v);

%
% The force exerted on the floating base by the links
%
vcross = vcross_mx(trunk_v);
trunk_f = - vcross' * ip.lf_trunk.tensor6D * trunk_v;
...
%
trunk_a = - inverse(ci.trunk_Ic) * trunk_f; % TODO inverse
