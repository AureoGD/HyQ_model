...
class CODEInitializationExpression : public CODELoggable
{
  private:
    ...
    static TRanrotBGenerator mRanrotB;
  ...
  public:
    ...
    static void randomInit(rg_uint32 seed)
    {
      if (seed > 0)
        mRanrotB.RandomInit(seed);
      else
        mRanrotB.RandomInit(gRanrotB.BRandom());
    }

    double evaluate()
    {
      mParser.SetExpr(mExpression);
      mParser.DefineConst("x", mRanrotB.Random());
