function [X,Xd,Q,RPY, ...
    ...
    time] = hyq_do_Rollout(theta,realtime)
...
if realtime % show stdes_th
    sim = odesim('hyq_green.xml');                         % Load configuration
    sim.realtime();
else % show nothing
    sim = odesim('hyq_green.xml','nodialog'); 
    % Load configuration
%     sim.realtime();     
end
...
%% pre-Control loop
for t = 0:sim.step():1 % one sec should be enough
