function [X,Xd,Q,RPY, ...
    ...
    time] = hyq_do_Rollout_right_back(theta,task,realtime)
...
%     sim = odesim('hyq2max_terrain.xml');                         % Load configuration
    sim = odesim('hyq2max_terrain_back.xml');                         % Load configuration
    sim.realtime();
else % show nothing
    sim = odesim('hyq2max_terrain_back.xml','nodialog'); 
    % Load configuration
%     sim.realtime();     
end
if realtime
...
%% pre-Control loop
for t = 0:sim.step():4 % one sec should be enough
