...
% cost_sel=2;
task = task_hyq_jump(N);

xmean = rand(N,1);    % objective variables initial point

% load('init_val_mean.mat');
% xmean=initial_val_mean;
% xmean = task.init';

%sigma = 0.00000005;          % coordinate wise standard deviation (step size)
%sigma = 0.000005;          % coordinate wise standard deviation (step size)
% sigma = 0.001;  % IH #########% coordinate wise standard deviation (step size)
% sigma = 0.00005;  % I2 #########% coordinate wise standard deviation (step size)
% sigma = 0.0005;  % IH #########% coordinate wise standard deviation (step size)
...
clock_array = clock;
save(['mu9_jumpINITrand_data_' date '-' num2str(clock_array(4)) '-' num2str(clock_array(5))  '-cost1' '-sigma5']);
toc
end
%  save(['mu9_jumpINITtask_data_' date '-' num2str(clock_array(4)) '-' num2str(clock_array(5)) '-cost2' '-sigma6']);
