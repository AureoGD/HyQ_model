function [X,Xd,Q,RPY, ...
    ...
    time] = hyq_do_Rollout_right_savefull(theta,task,realtime)
...
%     sim = odesim('hyq2max_terrain.xml');                         % Load configuration
    ...
    sim = odesim('hyq2max_terrain.xml','nodialog'); 
    % Load configuration
%     sim.realtime();     
end
% if realtime
...
X = zeros(3,size(time_bookeeping,2));  % positions (cartesian, absolute)
Xd = zeros(3,size(time_bookeeping,2)); % velocities (cartesian, absolute)
Q = zeros(4,size(time_bookeeping,2));  % quaternions (x,y,z,w)
RPY = zeros(3,size(time_bookeeping,2));
dofs = 12;
th = zeros(dofs,size(time_bookeeping,2)); % joint angles
thd = zeros(dofs,size(time_bookeeping,2)); % joint angles rates
u  = zeros(dofs,size(time_bookeeping,2)); % torques
...
for t = 0:sim.step():2 % one sec should be enough
    sensors = sim.sense();                        % Measure sensor values
        ...
        tif = tif + 1;
    % Book-keeping
    X(:,tif) =  [sensors(hyq_x); sensors(hyq_y); sensors(hyq_z)];
    Xd(:,tif)= [sensors(hyq_xd); sensors(hyq_yd); sensors(hyq_zd)];
    Q(:,tif)=  [sensors(hyq_q1); sensors(hyq_q2); sensors(hyq_q3); sensors(hyq_q4)];
    
    rpy = quat2rpy(Q(4,tif),Q(1,tif),Q(2,tif),Q(3,tif)); %p=[p [rpy.roll;rpy.pitch;rpy.yaw]];end 
    RPY(:,tif)= [rpy.roll; rpy.pitch; rpy.yaw]; 

    th(:,tif)= [ sensors(LF_HAA_th); sensors(LF_HFE_th); sensors(LF_KFE_th);
    ...
    thd(:,tif)= [ sensors(LF_HAA_thd); sensors(LF_HFE_thd); sensors(LF_KFE_thd);
...
if realtime
    pause();
    for t = 0:sim.step():5 % should be enought
        sensors = sim.sense();  
    ...
    thd(:,tif)= [ sensors(LF_HAA_thd); sensors(LF_HFE_thd); sensors(LF_KFE_thd);
        ...
        % actuation PD → torque
        actuators(LF_HAA_u) = K*(th(1,keep_end) - sensors(LF_HAA_th)) + D*(0 - sensors(LF_HAA_thd));
        actuators(LF_HFE_u) = K*(th(2,keep_end) - sensors(LF_HFE_th)) + D*(0 - sensors(LF_HFE_thd));
        actuators(LF_KFE_u) = K*(th(3,keep_end) - sensors(LF_KFE_th)) + D*(0 - sensors(LF_KFE_thd));
        
        actuators(RF_HAA_u) = K*(th(4,keep_end) - sensors(RF_HAA_th)) + D*(0 - sensors(RF_HAA_thd));
        actuators(RF_HFE_u) = K*(th(5,keep_end) - sensors(RF_HFE_th)) + D*(0 - sensors(RF_HFE_thd));
        actuators(RF_KFE_u) = K*(th(6,keep_end) - sensors(RF_KFE_th)) + D*(0 - sensors(RF_KFE_thd));
        
        actuators(LH_HAA_u) = K*(th(7,keep_end) - sensors(LH_HAA_th)) + D*(0 - sensors(LH_HAA_thd));
        actuators(LH_HFE_u) = K*(th(8,keep_end) - sensors(LH_HFE_th)) + D*(0 - sensors(LH_HFE_thd));
        actuators(LH_KFE_u) = K*(th(9,keep_end) - sensors(LH_KFE_th)) + D*(0 - sensors(LH_KFE_thd));
        
        actuators(RH_HAA_u) = K*(th(10,keep_end) - sensors(RH_HAA_th)) + D*(0 - sensors(RH_HAA_thd));
        actuators(RH_HFE_u) = K*(th(11,keep_end) - sensors(RH_HFE_th)) + D*(0 - sensors(RH_HFE_thd));
        actuators(RH_KFE_u) = K*(th(12,keep_end) - sensors(RH_KFE_th)) + D*(0 - sensors(RH_KFE_thd));
